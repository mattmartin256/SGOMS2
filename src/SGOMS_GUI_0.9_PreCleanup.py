...
class PlanningUnit:
    ...
    def __init__(self, theID="Planning Unit", theUnitTaskList=None, theFiringConditions=None, theBehaviour=None):
        ...
        if theBehaviour == None:
            ...
            ## This is now handled in SGOMS_Model.outputToACTR, not in the behaviour of PUs or UTs themselves 2014.08.05
...
class Method:
    ...
    def __init__(self, theID="Method", theOperatorList = None, theFiringConditions=None, theBehaviour=None):
        ...
        theFiringConditions should be a list of strings that represent the firing conditions of a production in ACT-R
        theBehaviour should be a list of strings that represent the behaviour of a production in ACT-R
        '''
...
class Operator:
    '''An SGOMS Operator, the lowest level unit of SGOMS theory'''
...
class Node:
    ...
    def getClosestNodeTypeLegacy(self, theNodeType):
        ...
        theNodeType should be an instance of a PUNode, UTNode, MNode, or ONode
        '''
        ...
        if isinstance(self, PUNode) and isinstance(theNodeType, PUNode):
            print "(Node.getClosestNodeType) self is of the same type (PUNode) as the supplied node, returning self:", self.label
            return self
        
        if isinstance(self, UTNode) and isinstance(theNodeType, UTNode):
            print "(Node.getClosestNodeType) self is of the same type (UTNode) as the supplied node, returning self:", self.label
            return self
        
        if isinstance(self, MNode) and isinstance(theNodeType, MNode):
            print "(Node.getClosestNodeType) self is of the same type (MNode) as the supplied node, returning self:", self.label
            return self
        
        if isinstance(self, ONode) and isinstance(theNodeType, ONode):
            print "(Node.getClosestNodeType) self is of the same type (ONode) as the supplied node, returning self:", self.label
            return self
    ...
    def getClosestNodeTypeHelperLegacy(self, theNodeList, everyConnectedNode, theNodeType, times=0):
        '''Helper method for getClosestNodeType
        Returns the closest node of the type theNodeType the current node is
        ...
        theNodeType is the original node supplied to getClosestNodeType, it is used to do the end-state checking,
        ...
        ## 4 checks for the end state: if it has found a node of the same type as theNodeType, return the node
        for item in theNodeList:
            ## If we're looking for a PUNode, and we've found one
            if isinstance(item, PUNode) and isinstance(theNodeType, PUNode):
                ## Reset each recursed flag back to False before returning
                for thing in everyConnectedNode:
                    thing.recursed = False
                print "(Node.getClosestNodeTypeHelper)", item.label, "is the closest PUNode to ", self.label
                return item      ## Return the node it found
            
            ## If we're looking for a UTNode, and we've found one
            if isinstance(item, UTNode) and isinstance(theNodeType, UTNode):
                ## Reset each recursed flag back to False before returning
                for thing in everyConnectedNode:
                    thing.recursed = False
                print "(Node.getClosestNodeTypeHelper)", item.label, "is the closest UTNode to ", self.label
                return item      ## Return the node it found
            
            ## If we're looking for an MNode, and we've found one
            if isinstance(item, MNode) and isinstance(theNodeType, MNode):
                ## Reset each recursed flag back to False before returning
                for thing in everyConnectedNode:
                    thing.recursed = False
                print "(Node.getClosestNodeTypeHelper)", item.label, "is the closest MNode to ", self.label
                return item      ## Return the node it found
            
            ## If we're looking for an ONode, and we've found one
            if isinstance(item, ONode) and isinstance(theNodeType, ONode):
                ## Reset each recursed flag back to False before returning
                for thing in everyConnectedNode:
                    thing.recursed = False
                print "(Node.getClosestNodeTypeHelper)", item.label, "is the closest ONode to ", self.label
                return item      ## Return the node it found
        ...
        if len(passList) < 1:
            print "(Node.getClosestNodeTypeHelper) passList is empty, returning None"
    ...
    def getHopsToRootNode(self):
        ...
        This method was used prior to getHopsToNodeType(theNode), which is the preferred method of doing things as of 2014.08.13
        The current method is now basically obsolete, but can be used for testing purposes
    ...
    def getHopsToNodeTypeLegacy(self, theNodeType):
        ...
        theNodeType should be a PUNode, UTNode, MNode, or ONode
        ...
        c.getHopsToNodeType(pUNode=PUNode()) --> returns 2'''
        ...
        if isinstance(self, PUNode) and isinstance(theNodeType, PUNode):
            print "(Node.getHopsToNodeType) self is of the same type (PUNode) as the supplied node, returning 0"
            return 0
        
        if isinstance(self, UTNode) and isinstance(theNodeType, UTNode):
            print "(Node.getHopsToNodeType) self is of the same type (UTNode) as the supplied node, returning 0"
            return 0
        
        if isinstance(self, MNode) and isinstance(theNodeType, MNode):
            print "(Node.getHopsToNodeType) self is of the same type (MNode) as the supplied node, returning 0"
            return 0
        
        if isinstance(self, ONode) and isinstance(theNodeType, ONode):
            print "(Node.getHopsToNodeType) self is of the same type (ONode) as the supplied node, returning 0"
            return 0
        ...
        print "(Node.getHopsToNodeType) hops to theNodeType from", self.label, "=", returnVar
        self.recursed = False   ## Reset the recursed flag to False at the end
        return returnVar
    
    def getHopsToNodeTypeHelperLegacy(self, theNodeList, everyConnectedNode, theNodeType, times=0):
        ...
        theNodeType is the original node supplied to getHopsToNodeType, it is used to do the end-state checking,
        and it is not modified in any way.
        theNodeType should be an instance of a PUNode, UTNode, MNode, or ONode
        ...
        if times > 19:
            ...
            for thing in everyConnectedNode:
                    thing.recursed = False
            print ("(Node.getHopsToNodeTypeHelper) Recursed 20 times, returning None")
            return None
        ...
        for item in theNodeList:
            ...
            if isinstance(item, PUNode) and isinstance(theNodeType, PUNode):
                ...
                for thing in everyConnectedNode:
                    thing.recursed = False
                print "(Node.getHopsToNodeTypeHelper)", self.label, "is ", times + 1, " hops from the closest PUNode, ", item.label
                return times+1      ## Return # of hops
            ...
            if isinstance(item, UTNode) and isinstance(theNodeType, UTNode):
                ...
                for thing in everyConnectedNode:
                    thing.recursed = False
                print "(Node.getHopsToNodeTypeHelper)", self.label, "is ", times + 1, " hops from the closest UTNode, ", item.label
                return times+1      ## Return # of hops
            ...
            if isinstance(item, MNode) and isinstance(theNodeType, MNode):
                ...
                for thing in everyConnectedNode:
                    thing.recursed = False
                print "(Node.getHopsToNodeTypeHelper)", self.label, "is ", times + 1, " hops from the closest MNode, ", item.label
                return times+1      ## Return # of hops
        ...
        ## and it has not found a node of the same type as theNodeType, return None (throw an exception)
        if len(passList) < 1:
            print "(Node.getHopsToNodeTypeHelper) passList is empty, returning None"
            ## Reset the recursed flags
            for item in everyConnectedNode:
                #print item.label
                item.recursed = False
    ...
    def getClosestNodeType(self, theNodeType):
        ...
        '''
        if self.nodeType == "PUNode" and theNodeType == "PUNode":
            print "(Node.getClosestNodeType) self is of the same type (PUNode) as the supplied node, returning self:", self.label
            return self
        
        if self.nodeType == "UTNode" and theNodeType == "UTNode":
            print "(Node.getClosestNodeType) self is of the same type (UTNode) as the supplied node, returning self:", self.label
            return self
        
        if self.nodeType == "MNode" and theNodeType == "MNode":
            print "(Node.getClosestNodeType) self is of the same type (MNode) as the supplied node, returning self:", self.label
            return self
        
        if self.nodeType == "ONode" and theNodeType == "ONode":
            print "(Node.getClosestNodeType) self is of the same type (ONode) as the supplied node, returning self:", self.label
            return self
        '''    
    ...
    def getClosestNodeTypeHelper(self, theNodeList, everyConnectedNode, theNodeType, times=0):
        ...
        for item in theNodeList:
            ...
            '''
            ## If we're looking for a PUNode, and we've found one
            if item.nodeType == "PUNode" and theNodeType == "PUNode":
                ## Reset each recursed flag back to False before returning
                for thing in everyConnectedNode:
                    thing.recursed = False
                print "(Node.getClosestNodeTypeHelper)", item.label, "is the closest PUNode to ", self.label
            ...
            ## If we're looking for a UTNode, and we've found one
            if item.nodeType == "UTNode" and theNodeType == "UTNode":
                ## Reset each recursed flag back to False before returning
                for thing in everyConnectedNode:
                    thing.recursed = False
                print "(Node.getClosestNodeTypeHelper)", item.label, "is the closest UTNode to ", self.label
            ...
            ## If we're looking for an MNode, and we've found one
            if item.nodeType == "MNode" and theNodeType == "MNode":
                ## Reset each recursed flag back to False before returning
                for thing in everyConnectedNode:
                    thing.recursed = False
                print "(Node.getClosestNodeTypeHelper)", item.label, "is the closest MNode to ", self.label
            ...
            ## If we're looking for an ONode, and we've found one
            if item.nodeType == "ONode" and theNodeType == "ONode":
                ## Reset each recursed flag back to False before returning
                for thing in everyConnectedNode:
                    thing.recursed = False
                print "(Node.getClosestNodeTypeHelper)", item.label, "is the closest ONode to ", self.label
    ...
    def getHopsToNodeTypeHelper(self, theNodeList, everyConnectedNode, theNodeType, times=0):
        ...
        for item in theNodeList:
            ...
            if item.nodeType == "ONode" and theNodeType == "ONode":
                ...
                for thing in everyConnectedNode:
                    thing.recursed = False
                print "(Node.getHopsToNodeTypeHelper)", self.label, "is ", times + 1, " hops from the closest ONode, ", item.label
                return times+1      ## Return # of hops
            '''
    ...
    def getEveryConnectedNodeHelper(self, theNodeList, everyConnectedNode, times=0):
        ...
        ## This is the check for end state, if it has gone 20 times, the function ends and returns None
        if times > 19:
            print "(Node.getEveryConnectedNodeHelper) Recursed 20 times, returning False"
            return None
        ...
        if len(passList) < 1:
            print "(Node.getEveryConnectedNodeHelper) passList is empty, returning everyConnectedNode:"
            for item in everyConnectedNode:
                print item.label
...
class UTNode(Node):
    ...
    def __init__(self, aLabel = "UTNode", aLocation = None, theIncidentEdges = None, thePUxUTRelation = None):
        ...
        ## This needs to be added to the SGOMS_Model somehow
        ## ^Done elsewhere, not in the init function
...
class MNode(Node):
    ...
    def __init__(self, aLabel = "MNode", aLocation = None, theIncidentEdges = None, theUTxMRelation = None):
        ...
        ## An empty UTxMRelation is created by default, to be populated later
        ## This needs to be added to the SGOMS_Model somehow
        ## ^Done elsewhere, not in the init function
    ...
    def updateMethod(self):
        ...
        based on the nodes the MNode is connected to
    ...
    def updateRelation(self):
        ...
        print "(MNode: ", self.label, ".updateRelation)"
        ...
        if isinstance(root, UTNode):
            ...
            self.uTxMRelation.location = self.order-1
...
class ONode(Node):
    ...
    def __init__(self, aLabel = "ONode", aLocation = None, theIncidentEdges = None, theMxORelation = None):
        ...
        ## An empty MxORelation is created by default, to be populated later
        ## This needs to be added to the SGOMS_Model somehow
        ## ^Done elsewhere, not in the init function
    ...
    def updateRelation(self):
        ...
        print "(ONode: ", self.label, ".updateRelation)"
...
class Edge:
    ...
    def draw(self, aPen):
        ...
        aPen.fillRect(self.returnMidpoint().x - 3, self.returnMidpoint().y - 3,     ##Draw a rectangle at the midpoint for selecting
                      6, 6)
...
class Graph:
    ...
    def addPUNodeAdvancedNew(self, aPlanningUnit, aPoint):
        ...
        self.SGOMS.addPlanningUnit(aPlanningUnit)   ## Adds the new UT to the list of Unit Tasks
    ...
    def addUTNodeAdvancedNew(self, aUnitTask, aPoint):
        '''Creates new UTNode, with aUnitTask and aPoint as the UnitTask and point of the UTNode
        Creates a blank PUxUTRelation, the Unit Task will be fed to the PUxUTRelation
        The PUxUTRelation will be fed to the UTNode, and added to SGOMS
        The UTNode will be added to self.nodes
        
        By default, this function creates a unique UT to be fed to the relation and SGOMS Model
        Future code should probably create a new PUxUTRelation, but not a new UT (in the case of duplicate UTs)
    ...
    def addMNodeAdvancedNew(self, aMethod, aPoint):
        '''Creates new MNode, with aMethod and aPoint as the Method and point of the MNode
        Creates a blank UTxMRelation, the Method will be fed to the UTxMRelation
        The UTxMRelation will be fed to the MNode, and added to SGOMS
        The MNode will be added to self.nodes
        
        By default, this function creates a unique Method to be fed to the relation and SGOMS Model
        Future code should probably create a new UTxMRelation, but not a new Method (in the case of duplicate Methods)
        
        I should make a new function in SGOMS that takes a UTxMRelation, and adds it to the list,
        and maybe also adds the Method to the methodList at the same time
        ^Nope, want to be able to have same pointers to same Method, for duplicate Methods'''
    ...
    def addONodeAdvancedNew(self, anOperator, aPoint):
        '''Creates new ONode, with anOperator and aPoint as the Operator and point of the ONode
        Creates a blank MxORelation, the Operator will be fed to the MxORelation
        The MxORelation will be fed to the ONode, and added to SGOMS
        The ONode will be added to self.nodes
        
        By default, this function creates a unique Operator to be fed to the relation and SGOMS Model
        Future code should probably create a new PUxUTRelation, but not a new Method (in the case of duplicate Methods)
    ...
    def addEdge(self, startNode, endNode):
        ...
        #### The below code is now obsolete, as is the method addEdgeHelper, do not use it (as of 2014.08.14)
    ...
    def addEdgeHelper(self, theUTNode):
        '''Obsolete as of 2014.08.14 - all functionality is handled by self.update, and this function is no longer used
    ...
    def deleteNode(self, theNode):
        ...
        ^2014.08.14, the above also applies to MNodes and ONodes,
        Methods and Operators are deleted from SGOMS regardless'''
...
class GraphEditor(JPanel, MouseListener, MouseMotionListener, KeyListener):
    ...
    def __init__(self, aGraph = None, aFrame = None):
        ...
        aFrame should be the frame that contains the GraphEditor panel'''
        ...
        if (event.getClickCount() == 2):
            ...
            if aNode == None:   ##If there was no node, check to see if it was an edge
                ...
                if anEdge == None: ##If no edge and no node clicked, create a new node
                    ...
                    ## Check to see which kind of SGOMS unit is selected
                    if self.graph.selectedSGOMSType == "PLANNING_UNIT":
                        print "(GraphEditor.mouseClicked) create new PUNode"
                        ...
                        dialog = SGOMSDialogTemplate(self.frame, "Create New Planning Unit", True, pU, event.getPoint())
                    
                    if self.graph.selectedSGOMSType == "UNIT_TASK":
                        print "(GraphEditor.mouseClicked) create new UTNode"
                        uT = UnitTask("UnitTask_" + str(len(self.graph.SGOMS.unitTaskList)+1))
                        dialog = SGOMSDialogTemplate(self.frame, "Create New Unit Task", True, uT, event.getPoint())
                        
                    if self.graph.selectedSGOMSType == "METHOD":
                        print "(GraphEditor.mouseClicked) create new MNode"
                        m = Method("Method_" + str(len(self.graph.SGOMS.methodList)+1))
                        dialog = SGOMSDialogTemplate(self.frame, "Create New Method", True, m, event.getPoint())
                        
                    if self.graph.selectedSGOMSType == "OPERATOR":
                        print "(GraphEditor.mouseClicked) create new ONode"
                        o = Operator("Operator_" + str(len(self.graph.SGOMS.operatorList)+1))
                        dialog = SGOMSDialogTemplate(self.frame, "Create New Operator", True, o, event.getPoint())
    ...
    def mouseDragged(self, event):
        ...
        if self.dragNode == None and self.dragEdge == None:
            print "(GraphEditor.mouseDragged), no dragNode or dragEdge; Translate everything"
    ...
    def update(self):
        ...
        print "(GraphEditor.update) Begin Graphics Update"
        self.requestFocus()
...
class SGOMSDialogPanel(JPanel):
    '''A panel that specifies the text fields etc. for creating/editing a new SGOMS Unit 
    (i.e. a PlanningUnit, UnitTask, Method, or Operator)
    (This panel only contains fields for name, firing conditions (5 fields), and behaviour (5 fields))
    The panel uses a GridBagLayout
    Used by SGOMSDialogTemplate
    ...
    is used by SGOMSDialogTemplate to fill in the name, firing conditions, and behaviour of all 4 SGOMS unit types'''
    ...
    def __init__(self, theSGOMSUnit):
        ...
        for textEntry in self.behaviourTextEntries:
            ...
            self.behaviourTextEntriesCounter += 1    ## The counter keeps track of where to put the component (row #)
...
class SGOMSDialogTemplate(JDialog):
    ...
    And as of 2014.08.08 the template is generic enough to create each of the 4 SGOMS unit types'''
    
    def __init__(self, theOwner = None, theTitle = "Create New SGOMS Node", isModal = True, theSGOMSUnit = None, thePoint = None):
        '''Initializes the SGOMSDialogTemplate, a template for the other dialog boxes
        ...
        print "SGOMSDialogTemplate Initiated"
        
        super(SGOMSDialogTemplate, self).__init__(theOwner, theTitle, isModal, windowClosing=self.cancelButtonPressed)
        ...
        #FDO print "(UnitTaskDialog.init) about to set the constraints for unitTaskSimplePanel"
        
        ## Set the constraints for the unitTaskSimplePanel, and add it to the frame
        constraints = GridBagConstraints()
        constraints.gridx = 0
        constraints.gridy = 0
        constraints.gridwidth = 1
        ...
        constraints.anchor = GridBagConstraints.WEST
        #self.layout.setConstraints(self.unitTaskSimplePanel, self.constraints)

        self.contentPane.add(self.SGOMSDialogPanel, constraints)
        #self.add(self.unitTaskSimplePanel, self.constraints)
        
        #FDO print "(UnitTaskDialog.init) Constraints for unitTaskSimplePanel set successfully, and added"
        ...
        #FDO print "(UnitTaskDialog.init) Constraints for buttonPanel set successfully, and added"
    ...
    def okButtonPressed(self, event):
        ...
        print "(SGOMSDialogTemplate.okButtonPressed())"
        ...
        self.owner.dialogFinishedGeneric(self.SGOMSUnit, self.point)
        print "SGOMSDialogTemplate disposed"
        self.dispose()
        
    def cancelButtonPressed(self, event):
        '''Defines what happens when the cancel button is pressed'''
        
        print "(SGOMSDialogTemplate.cancelButtonPressed())"      
...
class UnitTaskSimpleDialogPanel(JPanel):
    ...
    (This panel only contains fields for name, firing conditions, and behaviour)
    The panel uses a GridBagLayout
    Used by UnitTaskDialog'''
    
    def __init__(self, theUnitTask):
        '''Initializes the UnitTaskPanel with aUnitTask as a model that can feed it inputs
        
        aUnitTask should be a UnitTask'''
        
        super(UnitTaskSimpleDialogPanel, self).__init__()
        
        ## Store the unit task for modifying 
        self.unitTask = theUnitTask
        
        ## Store various variables to be input into the text fields
        ## These are the default strings that appear in the text fields upon startup
        ## The name variable
        if self.unitTask.ID == None:
            nameVar = "Name"
        else:
            nameVar = self.unitTask.ID

        ## The Firing Conditions variable (pick the first firing condition available)
        if len(self.unitTask.firingConditions) < 1:
            firingConditionVar = "Firing Condition"
        else:
            firingConditionVar = self.unitTask.firingConditions[0]

        ## The Behaviour variable (pick the first behaviour available)
        if len(self.unitTask.behaviour) < 1:
            behaviourVar = "Behaviour"
        else:
            behaviourVar = self.unitTask.behaviour[0]
        ...
        self.JLabels.append(self.nameLabel)
        self.JLabels.append(self.firingConditionsLabel)
        self.JLabels.append(self.behavioursLabel)
        
        ## Create The Text Entries
        self.nameEntry = JTextField(nameVar, 30)    ## Default text, and column width
        self.firingConditionsEntry = JTextField(30)
        self.behavioursEntry = JTextField(30)
        
        ## Add the Text Entries to the components List
        self.textEntries.append(self.nameEntry)
        self.textEntries.append(self.firingConditionsEntry)
        self.textEntries.append(self.behavioursEntry)
        ...
        ## Add labels to the panel, according to the GridBagLayout
        for label in self.JLabels:
            
            constraints = GridBagConstraints()
            constraints.gridx = 0  ## gridx = column# (always 0 for the Labels)
            constraints.gridy = self.JLabelsCounter   ## gridy = row# (equal to the # of previous components)
            
            ## Tinker with these later...
            constraints.gridwidth = 1;
            constraints.gridheight = 1;
            constraints.weightx = 1; ## grow horizontally
            constraints.weighty = 0; ## don't grow vertically
            ...
            #self.layout.setConstraints(label, self.layout)
            self.add(label, constraints)
            
            self.JLabelsCounter += 1
            
        print "(UnitTaskSimpleDialogPanel.init) Labels successfully added"
            
        ## Add textEntries to the panel, according to the GridBagLayout
        for textEntry in self.textEntries:
            
            constraints = GridBagConstraints()
            
            constraints.gridx = 1  ## gridx = column# (always 1 for the text entries)
            constraints.gridy = self.textEntriesCounter   ## gridy = row# (equal to the # of previous components)
            ...
            #self.layout.setConstraints(textEntry, self.layout)
            self.add(textEntry, constraints)
            
            self.textEntriesCounter += 1    ## The counter keeps track of where to put the component (row #)
            
        print "(UnitTaskSimpleDialogPanel.init) text entries successfully added"
    
        
    def reinitialize(self, aGraph):
        '''Reinitializes the panel when a new field is created
        This is used for adding new text fields for behaviours or firing conditions (not yet implemented)
        
        for each component in self.components, add the component'''
        
        pass

class UnitTaskComplexDialogPanel(JPanel):
    '''A panel that specifies the text fields etc. for creating/editing a new Unit Task
    ...
    def __init__(self, aGraph):
        ...
        super(UnitTaskComplexDialogPanel, self).__init__()
...
class UnitTaskDialog(JDialog):
    ...
    def __init__(self, theOwner = None, theTitle = "Create New Unit Task", isModal = True, theUnitTask = None):
        '''Initializes a UnitTaskDialog, the dialog that comes up when creating a new UnitTask
        
        theOwner is the client application that caused the dialog to open, must be a Frame of some sort (i.e. GraphEditorFrame)
        theTitle specifies the title of the pop-up window
        isModal specifies whether the window must be dealt with before other actions can happen 
        (True = must be dealt with)
        theUnitTask is the UnitTask that will be modified
        '''
        
        print "##### Unit Task Dialog Initiated #####"
        
        
        ## Notes to self: (2014.08.06)
        ## Create the main panel
        ## set its layout
        ## Add the other panels to the main panel according to its layout
        ...
        ## This is the panel that contains the text fields etc.
        self.unitTaskSimplePanel = UnitTaskSimpleDialogPanel(self.unitTask)       
        ...
        self.contentPane.add(self.unitTaskSimplePanel, constraints)
        #self.add(self.unitTaskSimplePanel, self.constraints)
        ...
        '''
        ## The original BoxLayout code for the dialog
        ## The UnitTaskPanel
        self.unitTaskPanel = UnitTaskComplexDialogPanel(self.unitTask)
    ...
    def okButtonPressed(self, event):
        ...
        Modifies the UnitTask, based on the info entered into the dialog box,
        and passes the UnitTask to the owner'''
        
        print "(UnitTaskDialog.okButtonPressed())"
        
        ##theID="Unit Task", theParentPlanningUnits=None, theFiringConditions=None, theBehaviour=None):
        ## The first text entry is the name, the second is the firing conditions, the third is the behaviour
        ## Right now these are hard-coded assuming there is only one firing condition and behaviour line, this could be changed later
        self.unitTask.ID = self.unitTaskSimplePanel.textEntries[0].getText()  ## Set the ID
        
        self.unitTask.firingConditions.append(self.unitTaskSimplePanel.textEntries[1].getText())   ## Add the firing conditions
        self.unitTask.behaviour.append(self.unitTaskSimplePanel.textEntries[2].getText())
        
        self.owner.dialogFinished(self.unitTask)
        self.dispose()
...
class ButtonPanel(JPanel):
    ...
    def createNewUnitTask(self, event):
        ...
        theUnitTask = UnitTask("finished")  ## By default new unit tasks will be called "finished"
        
        dialog = SGOMSDialogTemplate(self.frame, "Create New Unit Task", True, theUnitTask) ## theOwner, theTitle, isModal, theUnitTask
...
class GraphEditorFrame(JFrame, DialogClientInterface):
    ...
    def __init__(self, theTitle = "Title", theGraph = None):
        ...
        self.editor = GraphEditor(theGraph, self)
        self.buttonPanel = ButtonPanel(theGraph, self)
        ...
        ## Original code for adding the panel components
        self.setLayout(None)
        
        self.buttonPanel.setSize(100,400)
        ...
        '''
        ## Shiny new code for adding the panel components
    ...
    def dialogFinished(self, theUnitTask):
        ...
        ## This function creates a new Relation and adds it to SGOMS, and also adds theUnitTask to SGOMS
        self.graph.addUTNodeAdvancedNew(theUnitTask, Point(200,200))    ## For now, the location is arbitrary
        self.editor.update()
        
    def dialogFinishedGeneric(self, theSGOMSUnit, thePoint):
        '''Specifies what to do when an SGOMSDialogTemplate dialog box ends successfully
    ...
    def exportToACTR(self, event):
        ...
        ret = chooseFile.showDialog(self, "Select Location")
    ...
    def moreInformationSelected(self, event):
       ...
       event is the event object passed by the menu item'''
        ...
        JOptionPane.showMessageDialog(self, "Documentation can be found at: https://github.com/CarletonCognitiveModelingLab/SGOMS_GUI",
            "For More Information", JOptionPane.INFORMATION_MESSAGE)
...
'''
## SGOMS Test Code 4 (Testing Methods, Operators, and OutputToACTR)
## This will not use any features of the GUI
## The PUs and UTs are taken from SGOMS-ACT-R-BasicModel2
...
## Create the PUs and assign them behaviour etc.
## Create the UTs and assign them behaviour etc.
## Create the Methods and assign them behaviour etc.
## Create the Operators and assign them behaviour etc.
##
## Add the UTs to the PUs
## Add the Methods to the UTs
## Add the Operators to the Methods
## Create the relations and add the PUs, UTs, Methods, Operators etc.
## Add the PUs, UTs, Methods, Operators and Relations to the model
## Output to ACT-R

SGOMS = SGOMS_Model()
SGOMS.initialBehaviour.append("buffer_context.set('area:not_clear cover:not_taken status:not_started completed:no')")
...
## The Methods
aim_method = Method("aim_method")
shoot_method = Method("shoot_method")

aim_method.firingConditions.append("# When kill_enemies_unit_task fires")
aim_method.behaviour.append("# Aim at enemy")
aim_method.behaviour.append("pass")

shoot_method.firingConditions.append("# After aim_method fires")
shoot_method.behaviour.append("# Shoot at enemy")
shoot_method.behaviour.append("pass")


## The Operators
press_L_operator = Operator("press_L_operator")
press_R_operator = Operator("press_R_operator")

press_L_operator.firingConditions.append("# When aim_method fires")
press_L_operator.behaviour.append("# Press L")
press_L_operator.behaviour.append("pass")

press_R_operator.firingConditions.append("# When shoot_method fires")
press_R_operator.behaviour.append("# Press R")
press_R_operator.behaviour.append("pass")
...
## Add the Methods to the UTs
kill_enemies_unit_task.methodList.append(aim_method)
kill_enemies_unit_task.methodList.append(shoot_method)

## Add the Operators to the Methods
aim_method.operatorList.append(press_L_operator)
shoot_method.operatorList.append(press_R_operator)

## Create the Relations
## Should create a method in SGOMS_Model that updates the relations automatically
#pUxUTRelation1 = PUxUTRelation(1, take_area, advance_unit_task, 0)
#pUxUTRelation2 = PUxUTRelation(2, take_area, kill_enemies_unit_task, 1)
#pUxUTRelation3 = PUxUTRelation(3, take_area, finished, 2)

#pUxUTRelation4 = PUxUTRelation(4, hold_ground, find_cover_unit_task, 0)
#pUxUTRelation5 = PUxUTRelation(5, hold_ground, finished, 1)

## Change the DM strings manually for the PUxUTRelations
#pUxUTRelation1.DM_string = "planning_unit:take_area    cuelag:none          cue:start          unit_task:advance"
#pUxUTRelation2.DM_string = "planning_unit:take_area    cuelag:start         cue:advance        unit_task:kill_enemies"
#pUxUTRelation3.DM_string = "planning_unit:take_area    cuelag:advance       cue:kill_enemies   unit_task:finished"
#pUxUTRelation4.DM_string = "planning_unit:hold_area    cuelag:none          cue:start          unit_task:find_cover"
#pUxUTRelation5.DM_string = "planning_unit:hold_area    cuelag:start         cue:find_cover     unit_task:finished"


## Add the Relations, and update the PUxUTRelations (must keep track of an instance)
## addPUxUTRelationReturnSelf(self, theUnitTask, thePlanningUnit=None,  theLocation=0):

pUxUTRelation1 = SGOMS.addPUxUTRelationReturnSelf(advance_unit_task, take_area, 1)
pUxUTRelation2 = SGOMS.addPUxUTRelationReturnSelf(kill_enemies_unit_task, take_area, 2)
pUxUTRelation3 = SGOMS.addPUxUTRelationReturnSelf(finished, take_area, 3)

pUxUTRelation4 = SGOMS.addPUxUTRelationReturnSelf(find_cover_unit_task, hold_ground, 1)
pUxUTRelation5 = SGOMS.addPUxUTRelationReturnSelf(finished, hold_ground, 2)

SGOMS.updateRelation(pUxUTRelation1)
SGOMS.updateRelation(pUxUTRelation2)
SGOMS.updateRelation(pUxUTRelation3)
SGOMS.updateRelation(pUxUTRelation4)
SGOMS.updateRelation(pUxUTRelation5)

SGOMS.addUTxMRelationReturnSelf(aim_method, kill_enemies_unit_task, 1)
SGOMS.addUTxMRelationReturnSelf(shoot_method, kill_enemies_unit_task, 2)

SGOMS.addMxORelationReturnSelf(press_L_operator, aim_method, 1)
SGOMS.addMxORelationReturnSelf(press_R_operator, shoot_method, 1)


#SGOMS.pUxUTRelationList.append(pUxUTRelation1)
#SGOMS.pUxUTRelationList.append(pUxUTRelation3)
#SGOMS.pUxUTRelationList.append(pUxUTRelation4)
#SGOMS.pUxUTRelationList.append(pUxUTRelation5)


## Add the PUs, UTs, Methods and Operators to the model (do not add "finished")
SGOMS.planningUnitList.append(take_area)
SGOMS.planningUnitList.append(hold_ground)
...
SGOMS.methodList.append(aim_method)
SGOMS.methodList.append(shoot_method)

SGOMS.operatorList.append(press_R_operator)
SGOMS.operatorList.append(press_L_operator)
